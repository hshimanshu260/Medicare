...
const stripe = STRIP_KEY
  ? new Stripe(STRIP_KEY, { apiVersion: "2023-10-16" })
  : null;
...
//this will generate the frontend url
...
export const getAppointments = async (req, res) => {
  try {
    ...
    console.error("Get Appointments Errors:", err);
...
//to get appointments by patients
export const getAppointmentsByPatients = async (req, res) => {
  try {
    ...
    if (req.query.nobile) filter.mobile = req.query.mobile;
    ...
    console.error("Get AppointmentsByPatients Errors:", err);
    return res.status(500).json({
      success: false,
      message: "Server Error",
    });
...
export const createAppointment = async (req, res) => {
  try {
    ...
    if (numericFee === null || numericFee < 0) {
      return res.status(400).json({
        status: false,
        message: "Fees must be valid number.",
      });
    }
    ...
    if (existingBooking) {
      return res.status(409).json({
        success: false,
        message:
          "You already have an appointment with this doctor at selected slot.",
      });
    }
