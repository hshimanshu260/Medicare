...
//HELPER FUNCTIONS
//This function will convert time 12 hrs to number of minutes since midnight
...
//Removes duplicate slots and return the sort filter by time
...
//accept the objcet or the JSON string
...
//Convert the doctor data into plain text
...
export async function createDoctor(req, res) {
  try {
    ...
    if (req.file?.path) {
      const uploaded = uploadToCloudinary(req.file.path, "doctors");
...
export async function deleteDoctor(req, res) {
  try {
    ...
    if (existing.imagePublicId) {
      try {
        await deleteFromCloudinary(eexisting.imagePublicId);
      } catch (e) {
        console.warn("DeleteFromCloudinary warning:", e?.message || e);
      }
    }
    ...
    return res.json({ success: true, message: "Doctor deleted Sucessfully" });
  } catch (err) {
...
export async function toggleAvailability(req, res) {
  try {
    ...
    if (typeof doc.availability === "boolean")
      ...
      doc.availability =
        doc.availability === "Available" ? "Uavailable" : "Available";
    await doc.save();
    const out = normalizeDocForClient(doc.toObject());
    delete out.password;
    return res.json({
